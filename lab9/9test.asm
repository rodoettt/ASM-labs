...
;данные для приложения и окна
	...
	TITLENAME   DB 'Лабораторная работа № 9',0
...
START: ;\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\start
	...
	invoke 	GetModuleHandleA, 0 ; возвращает дескриптор файла, используемого, чтобы создать .exe
	mov   	[HINST], eax
	...
	mov   	[WC.CLSHINST], eax
	;пиктограмма окна
	invoke 	LoadIconA, 0, IDI_APPLICATION ; загружает указанный ресурс значка из исполняемого (.exe) файла, связанного с экземпляром приложения
	mov    	[WC.CLSHICON], eax
	;курсор окна
	invoke 	LoadCursorA, 0, IDC_ARROW ; Функция LoadCursor загружает заданный ресурс курсора из исполняемого (.EXE) файла
	mov	[WC.CLSHCURSOR], eax
	mov    	[WC.CLBKGROUND], 5h ;Цвет окна
	mov    	DWORD ptr [WC.CLMENNAME], MYMENU
	mov	DWORD ptr [WC.CLNAME], offset  CLASSNAME
	invoke	RegisterClassA, offset WC ;  регистрирует класс окна для последующего использования при вызове функции CreateWindow или CreateWindowEx
...
DrawGraph	proc _hWnd:DWORD, _hDC:DWORD ; вычисление и отрисовка графика функции

	
	invoke	CreatePen, 0, 2, 00ff00h
	mov	PenGreen, eax
	...
	;начальное значение координаты на графике
	fld	x
	fmul	scalex
	fist	xCor
	
	;определим количество повторов. Шаг у нас 0.01
	mov	eax, rRange
	sub	eax, lRange
	mov	ebx, 100
	mul	ebx
...
cycl:	;//////////////////////////////////////////////////////;цикл построения самого графика
        ; Вычисляем y = 2*x^5 - 3*x^2 + 5*x
        ...
        fadd        ; 2*x^5 - 3*x^2
        fld buf3
        fadd        ; 2*x^5 - 3*x^2 + 5*x
	
        fmul scaley ; масштабируем по Y
	
	fistp yCor   ; получаем целочисленную координату Y
        jmp     cont
...
NaN:
        ...
        inc     status
        jmp     cont2
	;вычисляем координаты на графике
cont:	cmp     status, 0
        je      cont3      
        mov     status, 0
        mov	eax, start_coord.X
...
line:	invoke 	LineTo, _hDC, XT, YT
noline:	invoke 	MoveToEx, _hDC, XT, YT, 0
...
LENSTR  PROC ; процедура для определения длины строки
        PUSH	EBP
        mov	EBP,ESP
        PUSH	esi

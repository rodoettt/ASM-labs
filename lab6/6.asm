...
copy:  		print 	curradd
    		mov     ax, cs
    		call    IntegerOut ; вывод адреса сегмента кода
    		cmp     mflag, 0
    		jnz     m9
    		mov     ax, cs ; загружаем адрес сегмента кода в ax
    		sub     ax, c ; узнаем смещение
    		add     ax, s
    		push    ax ; корректируем адрес

m9:		push    cs ; охраняет значение регистра cs (сегмент кода) в стеке
    		mov     bx, seg z
    		mov     ax, s
    		sub     bx, ax ; вычисление размера памяти
    		mov     ah, 48h ; выделение блока памяти
    		int     21h
    		jnc     m10
    		jmp     exit

m10:		mov     es, ax ; es указывает на новую область памяти
    		mov     cl, 3
    		shl     bx, cl ; bx содержит размер выделенного блока памяти в байтах.

    		mov     cx, bx ; счетчик для цикла
    		xor     bp, bp

m11:   		mov     ax, [bp] ; байт из текущей области памяти
    		mov     es:[bp], ax ; копируем в новую область памяти
    		add     bp, 2 ; переход к следующему слову в памяти
    		loop    m11

    		mov     ax, es ; адрес новой области памяти в ax
    		add     ax, c
    		sub     ax, s ; корректируем адрес
    		lea     si, m12
    		pushf ; сохраняет регистр флагов в стеке
    		push    ax ; сохраняет значение регистра ax (скорректированный адрес) в стеке
    		push    si
    		iret ; возврат из прерывания. Эта команда извлекает из стека регистр флагов, сегмент кода и смещение
...
C1:		cmp 	cs:delay, 0
  		...
  		add 	cs:delay, ax
for1:       
	    cmp 	byte ptr es:[si],192
		jae		co2 
		jmp 	co3 
co2: 	cmp 	byte ptr es:[si], 223
		jbe		co5
		jmp 	co3
co5:	sub 	byte ptr es:[si], 95 
co3: 		inc 	si 
		loop 	C1
		pop	es
		JMP write
